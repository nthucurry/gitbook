...
do
    ...
    az network vnet subnet show -g $resource_group -n $subnet --vnet-name $vnet \
    --query [].{subnet: name, ip: addressPrefix, nsg: networkSecurityGroup.name, route:routeTable.name} \
    -o json
done
